...
const unsigned char TREE = 'T'; //A tree, burns longer than grass, but has a low chance to ignite
const unsigned char BUSH = '*'; //A bush, doesn't bun as long as a tree, but high chance to ignite **maybe causes spaces around it to immidetaley combust
const unsigned char FIRE = 'F'; //Fire, ignites the spaces around it and burns for a limited amount of time
const unsigned char ASH = '.';
	...
	public:
		...
		void think() { //AI For fire
			if (out) return;
				...
				if (burntime < 0) {
					out = true;
					world[index(new_x,new_y)] = ASH;
					ff.pop_back(Fire(new_x,new_y));
					return;
				}
		}
...
int main()
{
	...
	while (true) {
		if (!game_on) 
			...
			mvprintw(SIZE_X,0,"Game Running. Fire: %i Trees: %i Bushes: %i Frame: %i",ff.size(), trees.size(), bushes.size(), frame++);
